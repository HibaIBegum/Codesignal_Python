...
def solution(grid):
    rows= grid # join all rows
    col = list(zip(*grid)) #join all columns 
    subs = []
    #converting the 3*3 sub grid to rows and saving in subs
    for i in range(0,7,3):
        for j in range(0,7,3):
            subs.append([grid[r][c] for r in range(i,i+3) for c in range(j,j+3)])
    #Compare the digit length by using set() which eleminates the duplicate numbers.
    def isvalid(arr):
